...
# root.iconbitmap("C:\\Users\\VICT6OR\\Pictures\\Project_game\\message.ico")
...
# #homepage view
# intro= Label(root, text= '       Welcome to Zen Live chat')
# intro.grid(row=0, column=0, columnspan=3)
# my_username=Entry(root, width=25)
# lbl=Label(root, text= 'Enter your username')
# lbl.grid(row=1, column=0)
# my_username.grid(row=1, column=1)
...
#         Label(root, text=f"{username.decode('utf-8')} joined live chat", fg='green').grid(row=0, column=0)
#         widget()
# def widget():
#     msg_lbl=Label(root, text="Message:")
#     msg_lbl.grid(row=1, column=0)
#     my_message= Entry(root, width=35)
#     my_message.grid(row=1, column=1)
...
#             try:
#                 while True:
#                     username_header= client_socket.recv(BYTESIZE)
...
#                     message_header= client_socket.recv(BYTESIZE)
...
# button_connect=Button(root,text='Join live chat', command=connect)
